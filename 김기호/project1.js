document.addEventListener('DOMContentLoaded', () => {
    ...
    function startTimer() { // 타이머 시작 함수
        timer = 60; // 타이머 초 단위 설정
        timerInterval = setInterval(() => { // 1초마다 실행 되는 인터벌 설정
            timer--; // 타이머 1초 감소
    ...
    function updateScoreDisplay() { // 점수 디스플레이 업데이트 함수
        scoreDisplay.textContent = `점수: ${score}`; // 점수 디스플레이 업데이트
    }
    ...
    function endGame() { // 게임 종료 함수
        endScreen.classList.remove('hidden'); // 알림 표시
        startButton.style.display = 'none'; // 시작 버튼 표시
        gameBoard.innerHTML = ''; // 게임 보드 초기화
        clearInterval(timerInterval); // 인터벌 해제
    }
});
